...
test.describe("QR Code Download", () => {
  ...
  test("should download PNG file via Generate & Download button", async ({
    page,
  }) => {
  ...
  test("should download button not be visible before QR generation", async ({
    page,
  }) => {
    // Go to fresh page
    await page.goto("/");
  ...
  test("should download different QR codes for different URLs", async ({
    page,
  }) => {
    const urlInput = page.locator('input[placeholder*="yoursite.com"]');
