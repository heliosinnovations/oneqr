...
test.describe('QR Code Generation', () => {
  ...
  test('should generate QR code for valid URL with https prefix', async ({ page }) => {
  ...
  test('should clear error message when entering valid URL after error', async ({ page }) => {
    const urlInput = page.locator('input[placeholder*="yoursite.com"]');
    const generateButton = page.locator('button:has-text("Generate QR Code")');
  ...
  test('should show loading state while generating', async ({ page }) => {
    ...
    await expect(generateButton).toContainText(/Generate QR Code|Generating.../);
