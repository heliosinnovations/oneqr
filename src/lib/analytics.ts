...
export const trackEvent = {
  ...
  qrGenerated: (type: "simple" | "advanced" | "bulk" | "template") => {
    posthog.capture("qr_generated", { type });
  },

  qrDownloaded: (
    format: "png" | "svg",
    type: "simple" | "advanced" | "bulk" | "template"
  ) => {
    posthog.capture("qr_downloaded", { format, type });
  },

  qrPrinted: (type: "simple" | "advanced" | "bulk" | "template") => {
    posthog.capture("qr_printed", { type });
  },

  qrCopied: (type: "simple" | "advanced" | "bulk" | "template") => {
